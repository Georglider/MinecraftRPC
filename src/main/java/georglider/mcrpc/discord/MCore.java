...
public class MCore {
    ...
    public static File downloadDiscordLibrary(Path path) throws IOException
    {
        ...
        Path discordSDKPath = Paths.get(path + "\\" + name + suffix);
        File f = discordSDKPath.toFile();
        if (f.exists()) {
            return f;
        }
...
//                temp.deleteOnExit();
                ...
                Files.copy(zin, discordSDKPath);

                // We are done, so close the input stream
                zin.close();

                // Return our temporary file
                return discordSDKPath.toFile();
            }
            // next entry
            zin.closeEntry();
        }
